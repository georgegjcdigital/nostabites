...
function RevealWord({ word, delay, italic, color, style: extraStyle }) {
  return (
    <span style={{ overflow:'hidden', display:'inline-block', verticalAlign:'bottom', paddingBottom:'0.18em', marginBottom:'-0.18em', paddingTop:'0.08em', marginTop:'-0.08em', ...extraStyle }}>
      <span style={{
    ...
    <section id="home" style={{
      ...
      <div ref={contentRef} style={{ maxWidth:'840px', position:'relative', zIndex:1, willChange:'transform, opacity' }}>
        ...
        <p style={{ margin:'0 0 44px', fontFamily:'var(--font-body)', fontSize:'clamp(1rem,2vw,1.2rem)', fontWeight:400, lineHeight:1.7, color:'var(--charcoal-600)', maxWidth:'560px', marginLeft:'auto', marginRight:'auto', marginBottom:'44px', animation:'nb-fade-up 0.8s var(--ease-out) 1.4s both' }}>
          Premium homemade Kerala snacks — Kozhukkatta, Pazham Pori, Cutlets and Sukiyan — crafted fresh with traditional recipes and pure ingredients.
        </p>
...
const PRODUCTS = [
  { name:'Kozhukkatta', traditional:'STEAMED RICE DUMPLINGS', desc:'Steamed rice flour dumplings filled with freshly grated coconut and dark jaggery.', badge:'Homemade', badgeVariant:'homemade', featured:true, img:'./assets/products/kozhukkatta.png' },
  { name:'Pazham Pori', traditional:'BANANA FRITTERS', desc:'Ripe banana slices in a light spiced batter, deep-fried to a crispy golden finish.', badge:'Bestseller', badgeVariant:'bestseller', img:'./assets/products/pazhampori.png' },
  { name:'Cutlets', traditional:'CHICKEN · BEEF · VEGETABLE', desc:'Spiced chicken, beef or vegetable patties with a crisp golden crumb — a Kerala teatime classic.', badge:'Fresh Today', badgeVariant:'fresh', img:'./assets/products/cutlets.png' },
  { name:'Sukiyan', traditional:'SWEET GRAM FRITTERS', desc:'Sweet deep-fried fritters with a spiced coconut and green gram filling — a beloved Kerala teatime treat.', badge:'Homemade', badgeVariant:'homemade', img:'./assets/products/sukiyan.png' },
];
  ...
  return (
    <section ref={sectionRef} id="about" style={{ background:'var(--cream-200)', padding: isMobile ? '0 0 48px' : '0 0 80px' }}>
      ...
      <div style={{ position:'relative', height: isMobile ? '300px' : '480px', overflow:'hidden' }}>
        ...
        />
        {/* Warm dark overlay */}
        <div style={{ position:'absolute', inset:0, background:'linear-gradient(to bottom, rgba(26,16,4,0.05) 0%, rgba(26,16,4,0.55) 45%, rgba(26,16,4,0.92) 100%)' }}/>
        {/* Text overlay */}
        <div className="nb-reveal" style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'flex-end', padding:'0 32px 56px', textAlign:'center' }}>
          <p style={{ margin:'0 0 10px', fontFamily:'var(--font-body)', fontSize:'11px', fontWeight:700, letterSpacing:'0.18em', textTransform:'uppercase', color:'rgba(240,191,24,1)', textShadow:'0 1px 8px rgba(0,0,0,0.5)' }}>Our Story</p>
          <h2 style={{ margin:'0 0 16px', fontFamily:'var(--font-display)', fontSize:'clamp(2.4rem,6vw,4rem)', fontWeight:600, fontStyle:'italic', color:'#fff', lineHeight:1.08, letterSpacing:'-0.02em', textShadow:'0 2px 8px rgba(0,0,0,0.7), 0 4px 32px rgba(0,0,0,0.5)' }}>
    ...
    <section id="contact" style={{ background:'var(--green-700)', padding: isMobile ? '56px 20px' : '80px 32px', textAlign:'center', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ maxWidth:'680px', margin:'0 auto', position:'relative' }} className="nb-reveal">
        ...
        <p style={{ margin:'20px 0 0', fontFamily:'var(--font-body)', fontSize:'12px', color:'rgba(255,255,255,0.45)', letterSpacing:'0.04em' }}>
          Usually responds within 1 hour · Delivery available in Cherthala &amp; nearby areas
        </p>
      </div>
    </section>
  );
      ...
      <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
        <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '2fr 1fr 1fr', gap: isMobile ? '28px' : '48px', marginBottom:'40px' }}>
              ...
              {[
                {
                  ...
                  icon: (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg>),
                },
                {
                  label: 'Facebook', href: '#',
                  bg: '#1877F2',
                ...
                <a key={label} href={href} aria-label={label} target="_blank" rel="noopener noreferrer"
                  style={{ width:'40px', height:'40px', borderRadius:'var(--radius-md)', background:'var(--charcoal-800)', color:'var(--charcoal-400)', display:'flex', alignItems:'center', justifyContent:'center', textDecoration:'none', flexShrink:0, transition:'transform 180ms cubic-bezier(0.34,1.56,0.64,1), box-shadow 200ms ease', cursor:'pointer' }}
                  onMouseEnter={e=>{ e.currentTarget.style.background=bg; e.currentTarget.style.color='#fff'; e.currentTarget.style.transform='scale(1.15)'; e.currentTarget.style.boxShadow='0 6px 20px rgba(0,0,0,0.4)'; }}
                  onMouseLeave={e=>{ e.currentTarget.style.background='var(--charcoal-800)'; e.currentTarget.style.color='var(--charcoal-400)'; e.currentTarget.style.transform='scale(1)'; e.currentTarget.style.boxShadow='none'; }}
                  onMouseDown={e=>{ e.currentTarget.style.transform='scale(0.88)'; e.currentTarget.style.transition='transform 80ms ease'; }}
                  onMouseUp={e=>{ e.currentTarget.style.transform='scale(1.15)'; e.currentTarget.style.transition='transform 180ms cubic-bezier(0.34,1.56,0.64,1), box-shadow 200ms ease'; }}
                >
                  {icon}
                </a>
              ))}
          ...
          <div>
            ...
            <nav style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
              {['Kozhukkatta','Pazham Pori','Cutlets','Sukiyan'].map(s => (
